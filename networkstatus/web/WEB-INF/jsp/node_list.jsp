    ...
    <body class="noheader">
        ...
        <div id="body">
            ...
            <div id="content">
                ...
                <fieldset>
                    <legend>Search Nodes</legend>

                    <form name="form1" method="post" action="${initParam.PROJECT_ROOT_PATH}/nodeman/node/search">
                ...
                <table width="100%">
                    <tbody>
                        ...
                        <c:forEach items="${nodeListResource.nodeResources}"
                                   var="node"
                                   varStatus="status"
                                   begin="0"
                                   end="1999"
                            ...
                            <tr>
                                ...
                                <td align="center"><c:out value="${node.description}" /></td>


                                <td align="center"><c:out value="${node.enabled}" /></td>
                                ...
                                <td align="center">
                                    ...
                                    &nbsp;
                                    <a href="javascript:formSubmitById('editFormId_${node.nodeID}')" title="Edit Node"> 
                                        <img src="${initParam.PROJECT_ROOT_PATH}/resources/icons/edit_32x32.png" alt="Checked" border="0" width="20" height="20" />
                                    </a>

                                    &nbsp;

                                    <a href="javascript:formSubmitById('updateEnabledFormId_${node.nodeID}')" onclick="return confirmChangeStatus();"  title="Change Status Of Node">
                                    ...
                                    &nbsp;
                                    <a href="javascript:formSubmitById('deleteFormId_${node.nodeID}')" onclick="return messageforadmin('${node.nodeID}');"  title="Delete Node">
                                        <img src="${initParam.PROJECT_ROOT_PATH}/resources/icons/trash_32x32.png" alt="Checked" border="0" width="20" height="20" />
                                    </a>
                                    &nbsp;

                                    <form action="${initParam.PROJECT_ROOT_PATH}/nodeman/node/changeenablestatus/submit" method="post" id="updateEnabledFormId_${node.nodeID}">
                                        <input type="hidden" name="${_csrf.parameterName}"
                                               value="${_csrf.token}" />
                                    ...
                                    <form action="${initParam.PROJECT_ROOT_PATH}/nodeman/node/edit" method="post" id="editFormId_${node.nodeID}">
                                        <input type="hidden" name="${_csrf.parameterName}"
                                               value="${_csrf.token}" />
                                        <input type="hidden" name="id"
                                               value="${node.nodeID}" />
                                    ...
                                    <form action="${initParam.PROJECT_ROOT_PATH}/nodeman/node/delete/submit" method="post" id="deleteFormId_${node.nodeID}">
                                        <input type="hidden" name="${_csrf.parameterName}"
                                               value="${_csrf.token}" />
                                        <input type="hidden" name="id"
                                               value="${node.nodeID}" />
                ...
                <div id="pagination" style="text-align: center;width: 100%" >
                    ...
                    <c:if test="${searchCriteria.pageNo+1<=nodeListResource.totalPages}">
                    ...
                    <c:if test="${searchCriteria.pageNo+2<=nodeListResource.totalPages}">
                    ...
                    <form action="${initParam.PROJECT_ROOT_PATH}/nodeman/node/search" method="post" id="paginationPreviousForm">
                        <input type="hidden" name="${_csrf.parameterName}"
                               value="${_csrf.token}" />
                    ...
                    <form action="${initParam.PROJECT_ROOT_PATH}/nodeman/node/search" method="post" id="paginationNextForm">
                        <input type="hidden" name="${_csrf.parameterName}"
                               value="${_csrf.token}" />
                        <input name="name" type="hidden" value="${searchCriteria.searchText}"/>
                        <input name="pageNo" type="hidden" value="${searchCriteria.pageNo+1}"/>
                    ...
                    <form action="${initParam.PROJECT_ROOT_PATH}/nodeman/node/search" method="post" id="paginationFirstForm">
                        <input type="hidden" name="${_csrf.parameterName}"
                               value="${_csrf.token}" />
                        <input name="name" type="hidden" value="${searchCriteria.searchText}"/>
                    ...
                    <form action="${initParam.PROJECT_ROOT_PATH}/nodeman/node/search" method="post" id="paginationLastForm">
                        <input type="hidden" name="${_csrf.parameterName}"
                               value="${_csrf.token}" />
                        <input name="searchText" type="hidden" value="${searchCriteria.searchText}"/>
                        <input name="pageNo" type="hidden" value="${nodeListResource.totalPages}"/>
                    </form>
